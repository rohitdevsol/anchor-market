...
use {
    ...
    anchor_market::{
        accounts::InitializeMarket, instruction::SetWinnerSideIx, state::WinningOutcome,
...
#[test]
fn test_set_winner_by_non_authority_fails() {
    ...
    svm.airdrop(&non_authority.pubkey(), 1_000_000_000).unwrap();
    ...
    let blockhash = svm.latest_blockhash();
    let msg =
        Message::new_with_blockhash(&[instruction], Some(&non_authority.pubkey()), &blockhash);
    let tx =
        VersionedTransaction::try_new(VersionedMessage::Legacy(msg), &[&non_authority]).unwrap();
    let res = svm.send_transaction(tx);
    assert!(res.is_err(), "set winner by non-authority should fail");
}
