...
use {
    ...
    anchor_market::{
        accounts::InitializeMarket, instruction::MergeTokensIx, ID as ANCHOR_MARKET_ID,
    },
    litesvm::LiteSVM,
...
#[test]
fn test_merge_zero_amount_fails() {
    ...
    let merge_amount: u64 = 0;
    let accounts = anchor_market::accounts::MergeToken {
        market,
        user: user.pubkey(),
        collateral_mint: usdc_mint,
        user_collateral,
    ...
    let tx = VersionedTransaction::try_new(VersionedMessage::Legacy(msg), &[&user]).unwrap();
    let res = svm.send_transaction(tx);
    assert!(res.is_err(), "merge with zero amount should fail");
}
