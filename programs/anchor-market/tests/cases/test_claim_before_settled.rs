...
#[test]
fn test_claim_before_winner_set_fails() {
    ...
    common::mint_to(&mut svm, &payer, &usdc_mint, &user_collateral, 1_000_000_000);
    ...
    let instruction = Instruction::new_with_bytes(
        program_id,
        &(anchor_market::instruction::ClaimRewardIx { market_id }).data(),
        accounts.to_account_metas(None)
    );
    ...
    let tx = VersionedTransaction::try_new(VersionedMessage::Legacy(msg), &[&user]).unwrap();
    let res = svm.send_transaction(tx);
    assert!(res.is_err(), "claim before winner set should fail");
}
